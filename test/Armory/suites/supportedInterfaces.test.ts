...
export default function suite() {
  ...
  it("should support supporting interfaces", async () => {
    ...
    expect(
      await ctx.armoryContract.supportsInterface(ERC165InterfaceId)
    ).to.equal(true);
  });
  ...
  it("should support ERC1155 Interface", async () => {
    ...
    expect(
      await ctx.armoryContract.supportsInterface(ERC1155InterfaceId)
    ).to.equal(true);
  });
  ...
  it("should support ContractURI Interface", async () => {
    ...
    expect(
      await ctx.armoryContract.supportsInterface(ContractURIInterfaceId)
    ).to.equal(true);
  });
  ...
  it("should support ERC2981 Interface", async () => {
    ...
    expect(
      await ctx.armoryContract.supportsInterface(ERC2981InterfaceId)
    ).to.equal(true);
  });
}
